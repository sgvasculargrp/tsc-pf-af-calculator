...
(function () {
  ...
  function rowCigna(code,i,ctx){ var m=look(code); if(!code)return null;
    ...
    var af; if(!NUM(pf))af=NA; else{ var afSum=(m.al||0)+(m.au||0); var baseAf=isStd?(afSum===0?round2(pf*0.25):afSum*.5):NaN; af=NUM(baseAf)?round2(baseAf*mult):NA; }
  ...
  var FOOTNOTES = {
    ...
    "CIGNA":[{title:"PF \u2014 Standard",body:"(MOH lower + upper) \u00D7 0.45 for standard-complexity procedures.",tag:"(LB+UB)\u00D70.45"},{title:"PF \u2014 Complex",body:"MOH upper range \u00D7 0.95 for rows toggled to 'Cpx'.",tag:"UB\u00D70.95"},{title:"Anaesthetist fee (AF)",body:"Standard AF = (MOH AF lower + upper) \u00D7 0.5 \u00D7 row multiplier. Where the MOH AF benchmark is not available (both bounds = S$0), AF defaults to 25% of the Surgeon Fee (PF) for that row. Complex rows show N/A for AF.",tag:"(LB+UB)\u00D70.5 / 25% fallback"},{title:"Same incision",body:"1st 100%, 2nd 50%, 3rd 25%, 4th+ 0%.",tag:"100/50/25/0"},{title:"Separate incision",body:"1st 100%, 2nd 95%, 3rd 90%, 4th+ 0%.",tag:"100/95/90/0"},{title:"Assistant surgeon fee",body:"Included in surgeon fee \u2014 not separately payable.",tag:"Not Payable"}],
